...
export default function Post({ params }) {
  ...
  useEffect(() => {
    const fetchPost = async () => {
      try {
        const response = await axios.get(
          `https://quizlytics-server-gold.vercel.app/blog/${id}`
        );
    ...
    const fetchOtherPosts = async () => {
      try {
        const response = await axios.get(
          "https://quizlytics-server-gold.vercel.app/allBlogs"
        );
  ...
  return (
    <div className="flex flex-col lg:flex-row justify-around items-start mt-10 container mx-auto gap-10 px-4">
      <div className="container mx-auto mt-10 px-4 py-8 bg-white rounded-lg shadow-lg max-w-4xl">
        ...
        <div className="mt-10">
          <Link href="/blogs" className="text-primary-color hover:underline">
            ← Back to Blog
          </Link>
        </div>
        ...
        <div className="mt-10 w-full">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {otherPosts.map((otherPost) => (
              <div
                key={otherPost._id}
                className="bg-white p-4 rounded-lg shadow-lg flex flex-col items-center md:items-start"
              >
                <Image
                  src={otherPost.photo}
                  alt={otherPost.title}
                ...
                />
                <h3 className="text-lg font-semibold text-primary-color text-center md:text-left">
                  {otherPost.title}
                </h3>
                <p className="text-sm text-gray-500 text-center md:text-left">
                  {otherPost.summary.slice(0, 50)}
                </p>
                <Link
                  href={`/blogs/${otherPost._id}`}
