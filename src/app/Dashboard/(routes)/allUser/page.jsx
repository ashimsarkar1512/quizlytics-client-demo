...
const AllUser = () => {
  ...
  const {
    ...
    queryFn: async () => {
      const { data } = await axios(
        `https://quizlytics-server-gold.vercel.app/allUsers`
      );
      return data;
    },
  ...
  if (roleLoading || userRoleLoading)
    return (
      <div>
        <LoadingSpinner />
      </div>
  ...
  const handleDelete = async (email) => {
    try {
      ...
      const response = await axios.delete(
        `https://quizlytics-server-gold.vercel.app/deleteUser?email=${email}`
      );
      if (response.data) {
  ...
  const handleRoleChange = async (email, newRole) => {
    try {
      const response = await axios.patch(
        `https://quizlytics-server-gold.vercel.app/updateUserRole`,
        {
          email,
          role: newRole,
        }
            ...
            <Table className="w-full text-md bg-white shadow-md rounded mb-4">
              <TableHeader>
                <TableRow className="border-b">
                  <TableHead className="text-left p-3 px-5 min-w-[150px]">
                    Name
                  </TableHead>
                  <TableHead className="text-left p-3 px-5 min-w-[200px]">
                    Email
                  </TableHead>
              ...
              <TableBody>{renderUsers("user")}</TableBody>
            </Table>
          </div>
        </TabsContent>
        ...
        <TabsContent value="teacher">
          <div className="px-3 py-4 flex justify-center">
            <Table className="w-full text-md bg-white shadow-md rounded mb-4">
              ...
              <TableBody>{renderUsers("teacher")}</TableBody>
            </Table>
          </div>
        </TabsContent>
        ...
        <TabsContent value="admin">
          <div className="px-3 py-4 flex justify-center">
            <Table className="w-full text-md bg-white shadow-md rounded mb-4">
              ...
              <TableBody>{renderUsers("admin")}</TableBody>
            </Table>
          </div>
        </TabsContent>
      </Tabs>
